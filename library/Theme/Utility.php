...
use \Municipio\Helper\Utility as UtilityHelper;
...
class Utility
{
    ...
    public function render()
    {
        //Init blade 
        $blade = new Blade(
            (array) UtilityHelper::$viewPaths, 
            (string) UtilityHelper::$cachePath
        );
    ...
    {
        if (!file_exists(UtilityHelper::$cachePath)) {
            if (!mkdir(UtilityHelper::$cachePath, 0764, true)) {
                throw new Exception("Could not create cache folder: " . UtUtilityHelperility::$cachePath);
            }
        }

        return (string) UtilityHelper::$cachePath; 
    }
    ...
    public function locateController($controller)
    {
        ...
        if(is_array(UtilityHelper::$controllerPaths) && !empty(UtilityHelper::$controllerPaths)) {

            foreach (UtilityHelper::$controllerPaths as $path) {
